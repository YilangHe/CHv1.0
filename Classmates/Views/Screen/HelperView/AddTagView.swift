...
struct AddTagView: UIViewControllerRepresentable {
    ...
    func makeUIViewController(context: UIViewControllerRepresentableContext<AddTagView>) -> UIViewController {
        return UIViewController() // Container on which UIAlertContoller presents
    }
    
    func updateUIViewController(_ uiViewController: UIViewController, context: UIViewControllerRepresentableContext<AddTagView>) {
        ...
        if self.showAlert {
            ...
            alert.addAction(UIAlertAction(title: NSLocalizedString("Submit", comment: ""), style: .default) { _ in
                ...
                alert.dismiss(animated: true) {
                    self.showAlert = false
                    if (!tags.contains(textString.trimmingCharacters(in: .whitespacesAndNewlines))) {
                        self.tags.insert(textString, at: 0)
                    } else {
                        print("cannot add duplicated tags")
                    }
    ...
    func makeCoordinator() -> AddTagView.Coordinator {
        Coordinator(self)
    }
    ...
    class Coordinator: NSObject, UITextFieldDelegate {
        ...
        var control: AddTagView
        
        init(_ control: AddTagView) {
